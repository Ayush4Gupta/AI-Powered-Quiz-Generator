...
# ---------- DELETE /quizzes/pdf/{job_id} ----------
@router.delete("/pdf/{job_id}", tags=["PDF Management"])
async def delete_pdf_job(job_id: str):
    """Delete/cancel a PDF processing job and clean up its results"""
    try:
        # Try to revoke the task if it's still running
        from app.background.tasks import celery_app
        celery_app.control.revoke(job_id, terminate=True)
        
        # Try to get and clean up the result
        res = AsyncResult(job_id)
        if res and res.status in ['SUCCESS', 'FAILURE']:
            # Clean up the result from backend
            res.forget()
        
        log.info("delete.pdf_job.success", job_id=job_id)
        
        return {
            "job_id": job_id,
            "message": "Job deleted/cancelled successfully",
            "status": "deleted"
        }
        
    except Exception as e:
        log.error("delete.pdf_job.failed", job_id=job_id, error=str(e))
        # Even if deletion fails, return success to avoid client errors
        return {
            "job_id": job_id,
            "message": f"Job deletion attempted (may have already completed): {str(e)}",
            "status": "attempted"
        }

# ---------- DELETE /quizzes/poll/{job_id} ----------  
@router.delete("/poll/{job_id}", tags=["Task Polling"])
async def delete_task_job(job_id: str):
    """Delete/cancel any task job (PDF, folder, article processing) and clean up its results"""
    try:
        # Try to revoke the task if it's still running
        from app.background.tasks import celery_app
        celery_app.control.revoke(job_id, terminate=True)
        ...
        log.info("delete.task_job.success", job_id=job_id)
        
        return {
            "job_id": job_id,
            "message": "Task deleted/cancelled successfully", 
            "status": "deleted"
        }
        
    except Exception as e:
        log.error("delete.task_job.failed", job_id=job_id, error=str(e))
        # Even if deletion fails, return success to avoid client errors
        return {
            "job_id": job_id,
            "message": f"Task deletion attempted (may have already completed): {str(e)}",
            "status": "attempted"
        }
...
async def generate_quiz(req: GenerateRequest):
    ...
    If use_all_content=True or topic is empty/None, generates quiz from all indexed content in the session.
    Otherwise, generates topic-specific quiz.
    """
    ...
    try:
        # Determine if we should use all content
        use_all_content = req.use_all_content or not req.topic or req.topic.strip() == ""
        effective_topic = req.topic if req.topic and req.topic.strip() else "All Indexed Content"
        ...
        # Validate session requirement for all-content mode
        if use_all_content and not session_filter:
            raise QuizError.validation_error("session_id is required when generating quiz from all content (use_all_content=True or empty topic)")
        
        quiz_result = generate_quiz_with_variants(
            effective_topic, req.num_questions, req.difficulty, req.employee_level, 
            req.num_variants, session_filter, use_all_content
        )
        
        if not quiz_result or not quiz_result.get("variants"):
            if use_all_content:
                raise QuizError.not_found("No questions could be generated from the indexed content in this session")
            else:
                raise QuizError.not_found("No questions could be generated for the given topic")
        ...
        log.error("generate.failed", 
                 topic=effective_topic if 'effective_topic' in locals() else req.topic, 
                 error=str(e))
